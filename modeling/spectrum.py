    ...
    def ryabkova(self, k):
        ...
        for j in range(n):
            ...
            if tmp.size == 0:
                break
            ind[j+1] = np.max(tmp)
            ryabkova[ind[j]: ind[j+1]] = self.piecewise_spectrum(j, k[ind[j]:ind[j+1]])

        ryabkova[ind[-1]:] = self.piecewise_spectrum(n, k[ind[-1]:])
